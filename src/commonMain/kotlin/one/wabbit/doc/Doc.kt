...
@Serializable
data class DocOps<out A>(val ops: LazyList<DocOp<A>>) {
...
@Serializable
sealed interface PageWidth {
    @Serializable
    data class AvailablePerLine(val lineLength: Int, val ribbonFraction: Double) : PageWidth {
...
@Serializable
sealed interface Doc<out Annotation> {
    @Serializable
    private data object Empty : Doc<Nothing> {
        override fun toString(): String = this.layoutCompact().toString()
    }

    @Serializable
    private data object Line : Doc<Nothing> {
        override fun toString(): String = this.layoutCompact().toString()
    }

    @Serializable
    private data class FlatAlt<Annotation>(val doc: Doc<Annotation>, val flat: Doc<Annotation>) :
        Doc<Annotation> {
        override fun toString(): String = this.layoutCompact().toString()
    }

    @Serializable
    private data class Text(val text: String) : Doc<Nothing> {
        init {
    ...
    @Serializable
    private data class Nest<Annotation>(val indent: Int, val doc: Doc<Annotation>) :
        Doc<Annotation> {
        override fun toString(): String = this.layoutCompact().toString()
    }

    @Serializable
    private data class Annotated<Annotation>(val annotation: Annotation, val doc: Doc<Annotation>) :
        Doc<Annotation> {
        override fun toString(): String = this.layoutCompact().toString()
    }

    @Serializable
    private data class Concat<Annotation>(val docs: List<Doc<Annotation>>) : Doc<Annotation> {
        override fun toString(): String = this.layoutCompact().toString()
    }

    @Serializable
    private data class Union<Annotation>(val left: Doc<Annotation>, val right: Doc<Annotation>) :
        Doc<Annotation> {
        override fun toString(): String = this.layoutCompact().toString()
    }
    ...
    //                val fittingPredicate: (Int, Int, Int, DocOps<Annotation>) -> Boolean = {
    // lineIndent, currentColumn, _initialIndentY, sdoc ->
    ...
    companion object {
        fun <A> annotate(annotation: A, doc: Doc<A>): Doc<A> = Annotated(annotation, doc)

        val empty: Doc<Nothing> = Empty

        fun <A> concat(docs: List<Doc<A>>): Doc<A> = Concat(docs)

        fun <A> concat(vararg docs: Doc<A>): Doc<A> = concat(docs.toList())
        ...
        fun <A> encloseSep(
            ...
            separator: String,
            docs: List<Doc<A>>,
        ): Doc<A> = encloseSep(text(open), text(close), text(separator), docs)

        /**
         * `hsep` concatenates all documents `xs` horizontally by putting a space between all
         * entries.
         */
        ...
        /**
         * `vsep` concatenates all documents `xs` above each other. If a `group`' undoes the line
         * breaks inserted by `vsep`, the documents are separated with a `space` instead.
         */
        ...
        fun <A> vsep(vararg docs: Doc<A>): Doc<A> = vsep(docs.toList())

        /**
         * `fillSep` concatenates the documents `xs` horizontally as long as it fits the page, then
         * inserts a `line` and continues doing that for all documents in `xs`. (`line` means that
         * if `group`ed, the documents are separated with a `space` instead of newlines. Use
         * `fillCat` if you do not want a `space`.)
         */
        ...
        /**
         * `sep` tries laying out the documents `xs` separated with `space`s, and if this does not
         * fit the page, separates them with newlines. This is what differentiates it from `vsep`,
         * which always lays out its contents beneath each other.
         */
        fun <A> sep(docs: List<Doc<A>>): Doc<A> = group(vsep(docs))

        /**
         * Lays out the document x with the current nesting level (indentation of the following
         * lines) increased by i. Negative values are allowed, and decrease the nesting level
         * accordingly.
         */
        ...
        /**
         * (group x) tries laying out x into a single line by removing the contained line breaks; if
         * this does not fit the page, or when a hardline within x prevents it from being flattened,
        ...
        //        AlreadyFlat  -> x
        //        NeverFlat    -> x
        fun <Ann> group(doc: Doc<Ann>): Doc<Ann> =
            when (doc) {
                is Union -> doc
                is FlatAlt ->
                    when (val changes = changesUponFlattening(doc.flat)) {
                    ...
                    when (val changes = changesUponFlattening(doc)) {
        ...
        private sealed interface FlattenResult<out A> {
            ...
            fun <B> map(f: (A) -> B): FlattenResult<B> =
                when (this) {
        ...
        private fun <A> changesUponFlattening(doc: Doc<A>): FlattenResult<Doc<A>> {
            when (doc) {
                is FlatAlt ->
                    return flatten(doc.flat)?.let { FlattenResult.Flattened(it) }
                ...
                is Annotated ->
                    return changesUponFlattening(doc.doc).map { Annotated(doc.annotation, it) }

                //            is Column -> FlattenResult.Flattened(Column { col ->
                // flatten(doc.f(col)) })
                //            is Nesting -> FlattenResult.Flattened(Nesting { nesting ->
                // flatten(doc.f(nesting)) })
                //            is WithPageWidth -> FlattenResult.Flattened(WithPageWidth { pageWidth
                // -> flatten(doc.f(pageWidth)) })
    ...
    //    //    -> Doc ann
    //    //    -> SimpleDocStream ann
    //    fun <A> layoutWadlerLeijen(fits: FittingPredicate<A>, pageWidth: PageWidth, doc: Doc<A>):
    // DocOps<A> {
    //
    //        //layoutWadlerLeijen
